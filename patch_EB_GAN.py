...
class AutoEncoder(chainer.Chain):
    ...
    def __init__(self, label):
        ...
        with self.init_scope():
            #too complesed?
            self.conv1 = L.ConvolutionND(ndim=3,in_channels=4, out_channels=5, ksize=3 ,pad=1)
            self.conv2 = L.ConvolutionND(ndim=3,in_channels=5, out_channels=5, ksize=3 ,pad=1)
            self.conv3 = L.ConvolutionND(ndim=3,in_channels=5, out_channels=5, ksize=3 ,pad=1)
            self.conv4 = L.ConvolutionND(ndim=3,in_channels=5, out_channels=5, ksize=3 ,pad=1)
            
            self.dconv1 = L.DeconvolutionND(ndim=3, in_channels=5, out_channels=5, ksize=2, stride=2 ,pad=1)
            self.dconv2 = L.DeconvolutionND(ndim=3, in_channels=4, out_channels=5, ksize=2, stride=2 ,pad=1)


            self.bn0=L.BatchNormalization(5)
            self.bn1=L.BatchNormalization(5)
            self.bn2=L.BatchNormalization(5)
            self.bn3=L.BatchNormalization(5)
            self.bn4=L.BatchNormalization(5)



    def __call__(self, x):

        h1 = F.relu(self.bn0(self.conv1(average_pooling_nd(x))))
        h2 = F.relu(self.bn1(self.conv2(average_pooling_nd(h1))))
        h3 = F.relu(self.bn2(self.conv3(h2)))
        h4 = F.relu(self.bn3(self.conv4(h3)))
        h5 = F.relu(self.bn4(self.dconv1(h4)))
        h6 = F.relu(self.dconv2(h5))
